...
# channel_location = pd.read_excel('channel_location.xls')
...
Net_number = 'EDAN'  # 选取网络类型 你可以选取其余网络 如EEGNet/DDC/DeepCoral等
...
file_handle.write('l_global = ' + str(l_global) + ', l_intra = ' + str(l_intra) + ', l_inter = ' + str(l_inter) + '\n')  # 记录权重
...
for seed in range(0, 1):  # 随机五个随机种子数
    ...
    Current_Datasets = 'RawData/'  # 数据集路径
        ...
        for k in range(0, len(subject_list)):
            for g in range(0, len(subject_list)):
                if subject_list[k] != subject_list[g] and Start_Task <= Task <= Terminal_Task:
                    ...
                    va_num = read_model_data(Basic_folder, Current_Datasets,
                    ...
                    for epoch in range(args.epochs):
                        ...
                        if epoch > 50:
                            ...
                            la_1 = 0
                        if epoch > 150:
                            la_2 = 2 / (1 + math.exp(-10 * ( (epoch - 150) / Epoch))) - 1
                        else:
                            la_2 = 0
                        ...
                        for i in range(0, num_iter):
                            ...
                            for key, net in net_dict.items():
                                ...
                                output, inter_global_loss, intra_ele_loss, inter_ele_loss = net[0](train_source_data_1, train_target_data_1)
                                clf_loss = net[2](output, train_source_label_1.long())
                                tr_loss = clf_loss + l_global * inter_global_loss + la_1 * l_intra * intra_ele_loss + la_2 * l_inter * inter_ele_loss
                                net[1].zero_grad()
                                tr_loss.backward()
                                net[1].step()
                                net[3].step()
                        ...
                        list.append(INTRA_ELE_LOSS, intra_ele_loss.item())
                        list.append(INTER_ELE_LOSS, inter_ele_loss.item())
                        ...
                        for validation_data, validation_label in validation_loader:
                            ...
                            va_pred, va_global_loss, va_intra_ele_loss, va_inter_ele_loss = net[0](validation_data, validation_data)
                    ...
                    #             + 'ACC/' + subject_1 + ' to ' + subject_2 + ' ACC')
                    # plt.close()
                    #
                    # # 绘制训练集、验证集的 CLF_Loss
                    # plt.figure()
                    ...
                    # plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
                    #             + 'CLF_LOSS/' + subject_1 + ' to ' + subject_2 + ' CLF Loss')
                    # plt.close()
                    #
                    # # 绘制ST_Global_Loss曲线图
                    # plt.figure()
                    # plt.plot(x_label, GLOBAL_LOSS)
                    # plt.xlabel('Epoch')
                    # plt.ylabel('INTER GLOBAL LOSS')
                    # plt.title('INTER GLOBAL LOSS / Epoch')
                    # plt.savefig('Figure/Train/Cross_' + str(Cross_Mission + 1) + '/'
                    #             + 'INTER_GLOBAL_LOSS/' + subject_1 + ' to ' + subject_2 + ' ST GLOBAL LOSS')
                    # plt.close()
                    #
                    # # 绘制INTRA_ELE_LOSS
                    # plt.figure()
                    # plt.plot(x_label, INTRA_ELE_LOSS)
                    # plt.xlabel('Epoch')
                    # plt.ylabel('ST ELE LOSS')
                    # plt.title('Source and Target-Intra Electrode Domain Loss / Epoch')
                    # plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
                    #             + 'INTRA_ELE_LOSS/' + subject_1 + ' to '+subject_2 + ' INTRA ELE LOSS')
                    # plt.close()
                    #
                    # # 绘制INTER_ELE_LOSS
                    # plt.figure()
                    # plt.plot(x_label, INTER_ELE_LOSS)
                    # plt.xlabel('Epoch')
                    # plt.ylabel('INTER ELE LOSS')
                    # plt.title('SSource and Target-Inter Electrode Domain Loss / Epoch')
                    # plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
                    #             + 'INTER_ELE_LOSS/' + subject_1 + ' to '+subject_2 + ' INTER ELE LOSS')
                    # plt.close()
                    #
                    # # 绘制TOTAL_LOSS
                    # plt.figure()
                    ...
                    # plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
                    #             + 'TOTAL_LOSS/' + subject_1 + ' to '+subject_2+' TOTAL LOSS')
                    # plt.close()
